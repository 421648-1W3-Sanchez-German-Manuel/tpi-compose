...
- **Namespace `secret/tpi/identity/*`** (the subsystem, not `users`). `shared/clients/<team>`
  and `<team>/*` are reserved for other teams; operators cannot read `<team>/*`.
...
2. **Onboard the other teams** (only `identity` is done): a team policy template
   (`read`/`write` on `secret/tpi/<team>/*`, `read` on `secret/tpi/shared/clients/<team>`),
   an onboarding script that creates the AppRole and returns `role_id` plus a response-wrapped
   `secret_id` (`-wrap-ttl=10m`), the Tailscale tag and auth key per team, and a kit
   (Agent template, sidecar example, README section) for running from an IDE.
